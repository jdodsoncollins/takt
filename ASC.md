...
Store Connect identifiers live in `.env` (`EXPO_PUBLIC_ASC_APP_ID`,
`EXPO_PUBLIC_BUNDLE_ID`, `EXPO_PUBLIC_APPLE_TEAM_ID`). They are not committed.

Auth uses an App Store Connect API key (`.p8`), not the Apple ID cookie session.
There is no key in this repo. After downloading one from
[App Store Connect → Integrations → API](https://appstoreconnect.apple.com/access/integrations/api):
...
asc auth login \
  ...
  --private-key ~/.asc/AuthKey_YOUR_KEY_ID.p8 \
  --network
asc auth status --validate
```

Individual keys omit `--issuer-id` and add `--key-type individual`.
Then list this app with `asc apps list` or `asc apps list --bundle-id "$EXPO_PUBLIC_BUNDLE_ID"`.
